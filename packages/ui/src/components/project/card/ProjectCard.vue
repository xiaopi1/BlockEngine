	...
	<SmartClickable class="w-full project-card-container">
		...
		<div v-if="layout === 'grid'" :class="[baseCardStyle, 'flex flex-col']">
			...
			>
				...
				/>
				<div v-else class="project-card-banner-fallback absolute inset-0" aria-hidden="true" />
			</div>
...
const baseCardStyle =
	'be-project-tile w-full h-full border-[1px] border-solid border-surface-4 overflow-hidden bg-surface-3 transition-all smart-clickable:outline-on-focus smart-clickable:highlight-on-hover'
...
.be-project-tile {
	...
	box-shadow:
		0 0.7rem 1.8rem rgb(9 28 38 / 11%),
		inset 3px 0 #3f9972;
	backdrop-filter: blur(14px) saturate(120%);
}

.be-project-tile:hover {
	transform: translateY(-2px);
	box-shadow:
		0 1rem 2.4rem rgb(9 28 38 / 16%),
		inset 3px 0 #3f9972;
}

.project-card-banner-fallback {
	background:
		radial-gradient(
			circle at 68% 28%,
		...
		repeating-linear-gradient(0deg, transparent 0 23px, rgb(255 255 255 / 5%) 23px 24px),
		repeating-linear-gradient(90deg, transparent 0 23px, rgb(255 255 255 / 5%) 23px 24px),
		color-mix(in srgb, var(--color-raised-bg) 82%, var(--color-brand) 18%);
}
