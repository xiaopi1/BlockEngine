...
; The custom setup shell is a fixed 940x620 window. The NSIS core remains
; responsible for safe installation, upgrades and uninstallation.
!define AXL_INSTALLER_UI_PATH "${__FILEDIR__}\..\..\..\target-original-ui\release\axolotl-installer-ui.exe"
...
!macro NSIS_HOOK_PREINSTALL
    ...
    ${If} ${FileExists} "$SMPROGRAMS\${PRODUCTNAME}.lnk"
        ...
        Pop $0
        ${If} $0 != "Admin"
			MessageBox MB_ICONINFORMATION|MB_OK "A legacy launcher installation was detected and requires administrator permission to upgrade. Windows will show a permission prompt next."
        ${EndIf}
